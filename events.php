    ...
    <div id="page-content-wrapper">
        <div class="container-fluid">
            ...
            <div class="card page-main-content" style="border:none;">
                <ul class="nav">
                    <li class="nav-item">
                        <a class="nav-link active-tab  nav-link-tab" href="events.php">Events</a>
                    </li>
                    <li class="nav-item">
                    ...
                    <li class="nav-item">
                        <a class="nav-link  nav-link-tab" href="myevents.php">My Events</a>
                    </li>
                </ul>
                <div>
                    ...
                    <div class="row">
                        ...
                        if ($allEvents) {
                            foreach ($allEvents as $row) {
                                if ($row['event_admin'] == $_SESSION['user_id']) {
                                    continue;
                                }
                                $is_interested = checkTableValue("events_members", ["event_id"], ["event_id" => $row['event_id'], "user_id" => $_SESSION['user_id'], "is_interested" => 1]);
                                $is_going = checkTableValue("events_members", ["event_id"], ["event_id" => $row['event_id'], "user_id" => $_SESSION['user_id'], "is_going" => 1]);
                                if (!empty($is_interested) || !empty($is_going)) {
                                    continue;
                                }
                                ?>
                                <div class="col-6 event-card" data-aos="flip-down">
                                    <div class="card mb-3 shadow p-3 mb-5 rounded event-card"
                                        ...
                                        <div class="row g-0">
                                            <div class="col-md-5">
                                                <div>
                                                    <img src="<?= base_url(!empty($row['event_cover']) ? $row['event_cover'] : ""); ?>"
                                                         alt="data" class="event-card-image"
                                                         style="width:15rem; height: 10rem;">
                                                </div>
                                            </div>
                                            <div class="col-md-7" style="height:10rem;">
                                                <div class="card-body">
                                                    <p class="text-center" style="font-size:12px;">
                                                        <b> <a
                                                                    href="event/<?= $row['slug'] . ".php"; ?>"><?= $row['event_title']; ?></a></b><br>
                                                        <?php
                                                        $desc = strip_tags($row['event_description']);
                                                        $ndesc = substr($desc, 0, 100);
                                                        echo $ndesc;
                                                        ?>...</p>

                                                    <br>
                                                    <div class="text-center ">
                                                        <button onclick="window.location='<?= base_url($page . ".php") . "?event_id=" . $row['event_id']; ?>&event_action=going'"
                                                                class="btn btn-danger btn-sm rounded-pill going"><i
                                                                    class="fas fa-hand-paper"></i>&nbsp;Going
                                                        </button>&nbsp;
                                                        <button onclick="window.location='<?= base_url($page . ".php") . "?event_id=" . $row['event_id']; ?>&event_action=interested'"
