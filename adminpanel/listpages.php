...
<div class="mt-5 mb-2"><strong><i class="fas fa-flag"></i>&nbsp;&nbsp;Page > List</strong></div> 
<br>    
            </div>
    ...
    <br>
      ...
      <table border="1" class="table table-bordered table-striped table-hover supTable list" id="Tabla">
                <tr> 
                    ...
                    <th onclick="sortTable('supTable', 1)">Page &nbsp;&nbsp;<div class="fas fa-arrow-up rotate "></div></th>
                    <th onclick="sortTable('supTable', 2)">Admin &nbsp;&nbsp;<div class="fas fa-arrow-up rotate "></div></th>
                    <th onclick="sortTable('supTable', 3)">Likes &nbsp;&nbsp;<div class="fas fa-arrow-up rotate "></div></th>
                    <th onclick="sortTable('supTable', 4)">Verified &nbsp;&nbsp;<div class="fas fa-arrow-up rotate "></div></th>
...
if($row)
{
  ...
  {
    $admin_id = $row['page_admin'];
    $query1 = "select * from users where user_id in (select page_admin from pages where page_admin = $admin_id)";
...
<tr >
      ...
      <td><a href="#" style="text-decoration: none; color:black;"><img src="<?php echo $row['page_picture'];?>" class="user-profile-photo">&nbsp;</a> 
        <?php echo $row['page_title'];?></td>
      <td><a href="#" style="text-decoration: none; color:black;"><img src="<?php echo $pic; ?>" class="user-profile-photo">&nbsp;<?php echo $name.' '.$lname; ?></a></td>
      <td><?php echo $row['page_likes']; ?></td>
      <td><button class="rounded-pill" style="background-color:#660033; color:white;">
        <?php 
        if ($row['page_verified'] ==1){
          echo 'Yes';
        }
        else if ($row['page_verified'] ==0){
          echo 'No';
        }
        ?>
      </button></td>
