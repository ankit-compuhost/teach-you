...
<div class="mt-5 mb-2"><strong><i class="fas fa-user"></i>&nbsp;&nbsp;User > List</strong></div> 
<br>    
            </div>
...
<td><a href="#" style="text-decoration: none; color:black;"><img src="<?php echo $row['user_picture']; ?>" class="user-profile-photo">&nbsp;</a><?php echo $row['user_firstname'].' '.$row['user_lastname']; ?></td>
<td><a href="#" style="text-decoration: none; color:black;"><?php echo $row['user_name']; ?></a></td>
<td><?php echo $row['user_registered'] ?></td>
<td><button class="rounded-pill" style="background-color:#660033; color:white;">
  <?php 
...
<?php
  ...
  echo "no event found";
    ...
    if($page > 1)
    {
     echo "<a href='listusers.php?page=1' class=' btn btn-outline-danger'>First</a>";
     echo "<a href='listusers.php?page=".($page - 1)."' class=' btn btn-outline-danger'><<</a>";
    }
    for($i=$start_loop; $i<=$end_loop+1; $i++)
    {     
if ($i == $page) {   
              echo "<a  href='listusers.php?page=".$i."' class='active btn btn-outline-danger' >$i</a>";   
          }               
          else  {   
              echo "<a href='listusers.php?page=".$i."' class='btn btn-outline-danger' role='group'>$i</a>";     
          }       }
    if($page <= $end_loop)
    {
     echo "<a href='listusers.php?page=".($page + 1)."' class=' btn btn-outline-danger'>>></a>";
     echo "<a href='listusers.php?page=".$total_pages."' class=' btn btn-outline-danger'>Last</a>";
    }
    ?>
</div>
</div>
